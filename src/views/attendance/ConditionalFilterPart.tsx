...
import { useGetAllDepartmentsLazyQuery, useGetCodesLazyQuery } from '@/types/generated/types';
...
const ConditionalFilterPart = ({ cnt }: IDateMemberCntProps) => {
  ...
  const [getCodesQuery, { data: codeData }] = useGetCodesLazyQuery({
    variables: {
      parents: ['contractType', 'position', 'WORKING_TYPE'],
    },
    onError: (err) => {
      alert('err');
    },
  ...
  const workingTypeOptions =
    (codeData?.codes &&
      codeData?.codes[2]?.codes?.map((code) => ({
        value: code?.code ?? '',
        label: code?.name ?? '',
      }))) ??
    [];
  ...
  return (
    <>
      <div className="w-7/12 max-w-full px-3 mt-0 lg:w-1/2">
        ...
        <div className="mt-[10px] -ml-3 py-3 rounded-lg bg-gray-100 p-6 w-[1600px]">
            ...
            <div className="flex justify-center">
              {workingTypeOptions.map((workType, idx) => (
                <div key={idx} className="mb-[0.125rem] mr-4 inline-block min-h-[1.5rem] pl-[1.5rem]">
                  ...
                  />
                  <label className="inline-block pl-[0.15rem] hover:cursor-pointer" htmlFor={`workingType${idx}`}>
                    {workType.label}
                  </label>
                </div>
              ))}
