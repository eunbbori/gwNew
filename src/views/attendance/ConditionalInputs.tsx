  ...
  return (
    <div className="relative flex flex-wrap px-3 mt-0">
      <div className="mt-[10px] -ml-3 py-3 rounded-lg bg-gray-100 p-6 w-full">
        <DatePickerRangeInput name="dateRange" control={args.control} title="기간" defaultValue={{ startDate: new Date(), endDate: new Date() }} />
        <div className="flex flex-wrap mt-5">
          ...
          <div className="w-1/4">
            <SelectInput {...defaultSelectAttributes} name="departmentId" selectOptions={deptOptions} title="부서" placeHolder="부서를 선택해주세요" />
          </div>
          <div className="w-1/4">
            <SelectInput {...defaultSelectAttributes} name="position" selectOptions={positionOptions} title="직급" placeHolder="직급을 선택해주세요" />
          </div>
        </div>
        {/* 근무형태 중복 체크박스 */}
        <div className="flex mt-5">
          <CheckBoxInput control={args.control} name="workingType" title="근무 형태" onChange={args.handleWorkingTypeChange} options={workingTypeOptions} />
        </div>
      </div>
    </div>
  );
